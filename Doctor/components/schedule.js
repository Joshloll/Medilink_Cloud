/**
 * Schedule Component
 * Renders the doctor schedule with calendar view - Clean Version
 */
      ...
      ${weekDays.map((day, index) => {
        ...
        return `
          <div class="relative h-full ${index === 2 ? 'bg-slate-50/30 dark:bg-slate-800/20' : ''}">
            ...
            ${dayEvents.length === 0 ? renderEmptyDay() : dayEvents.map(event => renderEvent(event)).join('')}
          </div>
        `;
      }).join('')}
    </div>
...
function renderEmptyDay() {
  return `
    <div class="flex flex-col items-center justify-center h-full text-center py-8">
      <span class="material-symbols-outlined text-2xl text-slate-300 mb-2">event_busy</span>
      <p class="text-xs text-slate-400">No appointments</p>
    </div>
  `;
}
  ...
  return `
    <div class="absolute top-[${topPosition}px] left-1 right-1 h-[${height}px] rounded ${typeColors[event.type]} border-l-4 border-l-500 p-2 cursor-pointer hover:shadow-md transition-shadow" onclick="handleEventClick('${event.id}')">
      <div class="flex justify-between items-start">
        <p class="text-xs font-bold text-slate-900 dark:text-white">${event.time}</p>
      ...
      <p class="text-xs text-slate-600 dark:text-slate-300">${event.notes || ''}</p>
    </div>
  `;
}
...
function setupCalendarInteractions() {
  // Add any additional calendar interaction setup here
  console.log('Calendar interactions set up');
}
