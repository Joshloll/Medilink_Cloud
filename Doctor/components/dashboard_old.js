/**
 * Dashboard Component
 * Renders the doctor dashboard with overview information
 */
  ...
  return `
    <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
      ...
      <button onclick="handleNewAppointment()" 
              class="btn btn-primary flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-bold shadow-sm shadow-blue-500/20">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <div class="card p-5 flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold text-slate-900 dark:text-white mt-1">$${stats.revenueThisMonth}</p>
          <p class="text-xs text-green-600 dark:text-green-400 mt-1">+18% vs last month</p>
        </div>
        <div class="size-10 rounded-full bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 flex items-center justify-center">
  ...
  return `
    <div class="lg:col-span-2 card p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
      <div class="flex items-center justify-between mb-4">
        ...
        <a href="Schedule.html" class="text-primary text-sm font-semibold hover:underline">View Full Schedule</a>
      </div>
      <div class="space-y-3">
        ${todayAppointments.length > 0 
          ? todayAppointments.map(apt => renderAppointmentCard(apt)).join('')
          : renderEmptyAppointmentsState()
        }
      </div>
    </div>
  `;
  ...
  return `
    <div class="flex items-center gap-4 p-3 bg-slate-50 dark:bg-slate-700/50 rounded-lg">
      <div class="size-10 rounded-full ${statusColors[appointment.status]} flex items-center justify-center">
        <span class="material-symbols-outlined">${statusIcons[appointment.status]}</span>
      </div>
      <div class="flex-1">
...
/**
 * Render empty appointments state
 */
function renderEmptyAppointmentsState() {
  return `
...
function renderActivityItem(record) {
  ...
  const iconColors = {
    ...
    'image': 'text-red-600 dark:text-red-300'
  };

  return `
    <div class="flex items-start gap-3">
      <div class="size-8 rounded-full ${iconColors[record.recordType]} dark:${iconColors[record.recordType].replace('text-', 'dark:bg-')} text-${iconColors[record.recordType].replace('text-', 'dark:bg-')} flex items-center justify-center flex-shrink-0 mt-1">
        <span class="material-symbols-outlined text-[16px]">${typeIcons[record.recordType]}</span>
      </div>
      <div class="flex-1">
...
function renderQuickActions() {
  return `
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      <div class="card p-5 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow cursor-pointer">
...
async function handleNewAppointment() {
  try {
    ...
    const newAppointment = {
      patientName: prompt('Patient Name:'),
